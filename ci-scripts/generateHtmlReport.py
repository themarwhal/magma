                ...
                for line in logfile:
                    ...
                    if cov_global_rate:
                        ...
                        if result is not None:
                            ...
                            self.file.write('     </tr>\n')
                        result = re.search(
                            r' functions.*: (.+)% \(([0-9]+) of ([0-9]+) functions', line)
                        if result is not None:
                            percentage = result.group(1)
...
while len(argvs) > 1:
    ...
    if re.match(r'^\-\-help$', myArgv, re.IGNORECASE):
        sys.exit(0)
    elif re.match(r'^\-\-job_name=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(r'^\-\-job_name=(.+)$', myArgv, re.IGNORECASE)
        HTML.job_name = matchReg.group(1)
    elif re.match(r'^\-\-job_id=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(r'^\-\-job_id=(.+)$', myArgv, re.IGNORECASE)
        HTML.job_id = matchReg.group(1)
    elif re.match(r'^\-\-job_url=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(r'^\-\-job_url=(.+)$', myArgv, re.IGNORECASE)
        HTML.job_url = matchReg.group(1)
    elif re.match(r'^\-\-git_url=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(r'^\-\-git_url=(.+)$', myArgv, re.IGNORECASE)
        HTML.git_url = matchReg.group(1)
    elif re.match(r'^\-\-git_src_branch=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(
            r'^\-\-git_src_branch=(.+)$',
            myArgv,
            re.IGNORECASE)
        HTML.git_src_branch = matchReg.group(1)
    elif re.match(r'^\-\-git_src_commit=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(
            r'^\-\-git_src_commit=(.+)$',
            myArgv,
            re.IGNORECASE)
        HTML.git_src_commit = matchReg.group(1)
    elif re.match(r'^\-\-git_src_commit_msg=(.+)$', myArgv, re.IGNORECASE):
        # Not Mandatory
        matchReg = re.match(
            r'^\-\-git_src_commit_msg=(.+)$',
            myArgv,
            re.IGNORECASE)
        HTML.git_src_commit_msg = matchReg.group(1)
    elif re.match(r'^\-\-git_merge_request=(.+)$', myArgv, re.IGNORECASE):
    ...
    elif re.match(r'^\-\-git_target_branch=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(
            r'^\-\-git_target_branch=(.+)$',
            myArgv,
            re.IGNORECASE)
        HTML.git_target_branch = matchReg.group(1)
    elif re.match(r'^\-\-git_target_commit=(.+)$', myArgv, re.IGNORECASE):
        matchReg = re.match(
            r'^\-\-git_target_commit=(.+)$',
            myArgv,
            re.IGNORECASE)
